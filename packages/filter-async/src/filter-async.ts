 ...
 */
export function filterByPromise<T>(
  ...
  return pipe(
    filterAsync(
      (data: T, index: number) => from(predicate(data, index)),
      parallel,
      maxConcurrent,
    ),
  );
...
export function filterAsync<T>(
  ...
  return parallel
    ? filterAsyncSequential(predicate)
    : filterAsyncParallel(predicate, maxConcurrent);
}
 ...
 */
function filterAsyncSequential<T>(
  predicate: Predicate$<T>,
): MonoTypeOperatorFunction<T> {
  return pipe(
 ...
 *                      Warning: Only active when parallel is true!
 */
function filterAsyncParallel<T>(
  predicate: Predicate$<T>,
  maxConcurrent: number = Number.POSITIVE_INFINITY,
): MonoTypeOperatorFunction<T> {
  return pipe(
...
function filterDataAndRemoveContainer<T>(): OperatorFunction<
  FilterContainer<T>,
  T
> {
  return pipe(

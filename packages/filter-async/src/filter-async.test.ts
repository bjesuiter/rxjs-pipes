  ...
  it("Correct element ordering after filtering - multiple rounds", (done) => {
    ...
    for (let i = 0; i < roundCount; i++) {
      const observable = range(1, 10)
        .pipe(
          ...
          bufferCount(5),
          map((result: number[]) =>
            difference(result, comparableResult).length === 0
          ),
        );
    ...
    forkJoin(observables)
      .subscribe((results: boolean[]) => {
        const singleResult = results.reduce((prev: boolean, curr: boolean) =>
          prev && curr
        );
...
describe("Filter Async Parallel", () => {
  ...
  it.todo("Filter runs parallel", (done) => {
        ...
        filterByPromise((value) => {
          switch (value) {
            case "a":
              ...
              return promiseDelay(
                moment.duration(3, "s").asMilliseconds(),
                true,
              );
            case "b":
              return promiseDelay(
